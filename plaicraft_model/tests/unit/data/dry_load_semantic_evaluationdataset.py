"""Dry-load a single batch using SemanticEvaluationDataset on real data.

Run:
    python tests/unit/data/dry_load_semantic_evaluationdataset.py

Paths can be set via environment variables (.env file):
- DATASET_PATH: Path to the processed dataset
- EVALUATION_METADATA_DB_PATH: Path to the evaluation metadata database
\"\"\"
...
SEMANTIC_EVALUATION_DATABASE_PATH = Path(os.getenv("EVALUATION_METADATA_DB_PATH", ""))
...
def main():
    if not DATASET_PATH.exists():
        ...
        return
    if not SEMANTIC_EVALUATION_DATABASE_PATH.exists():
        print(f"Semantic evaluation DB path not found: {SEMANTIC_EVALUATION_DATABASE_PATH}")
        return

    ds = SemanticEvaluationDataset(
        dataset_path=str(DATASET_PATH),
        semantic_evaluation_db_path=str(SEMANTIC_EVALUATION_DATABASE_PATH),
    )
    ...
    context_tail_frames = int(os.getenv("SEMANTIC_EVALUATION_CONTEXT_TAIL_FRAMES", "10"))

    now = datetime.now()
    timestamp = now.strftime("%Y%m%d_%H%M%S")
    base_output_dir = Path("logs") / "decoded_examples" / f"semantic_evaluation_{timestamp}"
    base_output_dir.mkdir(parents=True, exist_ok=True)

    print(f"=== Loading SemanticEvaluationDataset batches to {base_output_dir} ===\n")
